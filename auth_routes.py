...
from schemas import ContadorSchema, ClienteSchema, LoginSchema
...
class AuthRoutes:
    def __init__(self):
        ...
        self.router.add_api_route("/criar_conta", self.criar_conta, methods=["POST"])
        self.router.add_api_route("/login", self.login, methods=["POST"])
        self.router.add_api_route("/login-form", self.login_form, methods=["POST"])
        self.router.add_api_route("/refresh", self.use_refresh_token, methods=["GET"])

    def criar_token(self, id_usuario: int, duracao_token=timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)):
        data_expiracao = datetime.now(tz=timezone.utc) + duracao_token
        dic_info = {"sub": str(id_usuario), "exp": data_expiracao}
        return jwt.encode(dic_info, SECRET_KEY, ALGORITHM)

    def autenticar_usuario(self, login, senha, session):
    ...
    async def criar_conta(self, contador_schema: ContadorSchema, session: Session = Depends(pegar_sessao)):
    ...
    async def login(self, login_schema: LoginSchema, session: Session = Depends(pegar_sessao)):
        usuario = self.autenticar_usuario(login_schema.login, login_schema.senha, session)
        if not usuario:
        ...
        access_token = self.criar_token(usuario.id)
        refresh_token = self.criar_token(usuario.id, duracao_token=timedelta(days=7))
        return {
           "email": usuario.email,
    ...
    async def login_form(self, dados_formulario: OAuth2PasswordRequestForm = Depends(), session: Session = Depends(pegar_sessao)):
        usuario = self.autenticar_usuario(dados_formulario.username, dados_formulario.password, session)
        if not usuario:
            raise HTTPException(status_code=400, detail="Email nao cadastrado ou credenciais invalidas")
        access_token = self.criar_token(usuario.id)
        return {"access_token": access_token, "token_type": "bearer"}

    async def use_refresh_token(self, usuario: Contador = Depends(verificar_token)):
        access_token = self.criar_token(usuario.id)
        return {"access_token": access_token, "token_type": "bearer"}
